...
export class ShiftsService {
    ...
    getCandidateAvailability(): Promise<CalendarEvent[]> {
        ...
        */
        ...
        Promise.all([
            this.bridge.httpGET(`/query/PersonCustomObjectInstance1?fields=text1,text2,date1&where=candidate.id=${params.paramsMap.get('EntityID')[0]}`),
            this.bridge.httpGET(`/query/JobSubmission?fields=dateBegin&where=candidate.id=${params.paramsMap.get('EntityID')[0]}`),
            this.bridge.httpGET(`/query/Placement?fields=dateBegin&where=candidate.id=${params.paramsMap.get('EntityID')[0]}`)
